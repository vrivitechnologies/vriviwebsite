            ...
            <div className="flex flex-col md:flex-row items-center  justify-between gap-12">
                ...
                <div className="md:w-2/3 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <button className="px-6 py-3 bg-blue-500 text-white font-semibold rounded-md  transition shadow-lg">
                    ...
                    <div className=" grid grid-cols-1 sm:grid-cols-4 gap-3">
                        {Productcontent.stats.map((stat, idx) => (
                            <div
                                key={idx}
                                className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center shadow-md hover:bg-white/20 transition"
                            >
                                <div className="text-sm text-start opacity-80">{stat.label}</div>
                                <div className="text-xl font-medium  text-start text-white">{stat.value}</div>
                            </div>
                        ))}
                    </div>
                </div>
...
const HeroWithImage = () => {
    ...
    return (
        <section className="relative h-[580px] flex justify-center items-center overflow-hidden bg-blue-900 text-white">
            {/* Slides wrapper */}
            <div
            ...
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-3">
                {slides.map((_, idx) => (
                    <button
                        ...
                        className={`w-2 h-2 rounded-full ${idx === currentSlide % slideCount ? "bg-white" : "bg-gray-300"
                            }`}
                    />
                ))}
            </div>
